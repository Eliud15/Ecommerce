...
export const Capuchino = () => {
  const navegador = useNavigate();
    const [capuchino, setCapuchino] = useState([]);
    useEffect(() => {
      fetch("http://localhost:3000/capuchino")
  ...
  return (
    <>
    <Header/>
    <h1 className='text-center m-5'>Capuchinos</h1>
    <main
    className={`d-flex flex-wrap align-items-center justify-content-center mb-5 ${cardCss.main}`}
  >
    {capuchino.map((capuchino) => (
      <div key={capuchino.id} className={`m-2 p-2  ${cardCss.cardstye}`}>
        ...
        <h5 className={`text-center fw-semibold`}>{capuchino.nombre}</h5>
        <p className={`fw-semibold ${cardCss.p}`}>{capuchino.descripcion}</p>
